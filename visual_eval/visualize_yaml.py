...
def visualize_yaml(evaluation, folder, x, y, max_bound, ymin=0, ymax=1,
    ...
    for filename in os.listdir(folder):
        x_data, y_data, y_pos, z_labels = open_file(evaluation, filename,
            folder, x_data, y, y_data, y_pos, z_labels)
    ...
    multiple_plots(x_data, y_data, y_pos, max_bound-run_mean, ymin, ymax, title,
        x, y, z_labels)

def running_mean(x, N):
    return np.convolve(x, np.ones((N,))/N)[(N-1):]

def open_file(evaluation, filename, folder, x_data, y, y_data, y_pos, z_labels):
    ...
    if os.path.isdir(os.path.join(folder,filename)):
        ...
        for file in os.listdir(new_folder):
            temp_x, temp_y, temp_y_pos, temp_z = open_file(evaluation, file,
                new_folder, temp_x, y, temp_y, temp_y_pos, temp_z)
...
if __name__ == "__main__":
    ...
    visualize_yaml(args.measure, args.folder_name, args.x_label, args.y_label,
        args.max_bound_x, args.min_bound_y, args.max_bound_y, args.title,
        args.run_mean)
